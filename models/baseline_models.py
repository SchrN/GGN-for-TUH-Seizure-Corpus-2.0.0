...
class DCRNNModel_classification(nn.Module):
    ...
    def forward(self, input_seq, stage='test'):
        """
        Args:
...
class CNNNet(nn.Module):
    def __init__(self, args, out_mid_features=False):
        ...
        self.args = args
        self.out_mid_features = out_mid_features
        print("args-feature_len", args.feature_len)
        print("args-encoder_hid_dim", args.encoder_hid_dim)
...
class BasicCNN(nn.Module):
    def __init__(self,args, num_classes):
        ...
        self.args = args
        self.conv1 = nn.Conv2d(12, 24, 3)   #input,output,kernel,padding,stride
        self.pool  = nn.MaxPool2d(2,2)
        self.conv2 = nn.Conv2d(24, 48, 3)
        self.fc1 = nn.Linear(48*3*7, num_classes)

        self.relu = nn.ReLU()

    def forward(self, x, stage='test'):
        # input: torch.Size([32, 12, 20, 34])
...
##-------------------------- Basic CNN2  --------------- ##
#1 more conv layer and multiple fully connected layer
...
##--------------------------Mobilenet v.2--------https://github.com/jmjeon94/MobileNet-Pytorch/blob/master/MobileNetV2.py--------##

##bottleneck if expansion factor = 1   dwise =>conv1 , else conv1->dwise->conv(norelu)
## 


def dwise_conv(in_dim, stride=1):
    return (
        nn.Sequential(
            #depthwise
            nn.Conv2d(in_dim, in_dim, kernel_size=3, padding=1, stride=stride, groups=in_dim, bias=False),
            nn.BatchNorm2d(in_dim),
            nn.ReLU6(inplace=True),
        )
    )

def conv1x1(in_dim, out_dim):
    return (
        nn.Sequential(
            nn.Conv2d(in_dim, out_dim, kernel_size=1, padding=0, stride=1, bias=False),
            nn.BatchNorm2d(out_dim),
            nn.ReLU6(inplace=True)
        )
    )

def conv3x3(in_dim, out_dim, stride):
    return (
        nn.Sequential(
            nn.Conv2d(in_dim, out_dim, kernel_size=3, padding=1, stride=stride, bias=False),
            nn.BatchNorm2d(out_dim),
            nn.ReLU6(inplace=True)
        )
    )
...
class Bottleneck(nn.Module):
    def __init__(self, in_dim, out_dim, expand_ratio, stride):
        ...
        layers = []
        if expand_ratio != 1:
            layers.append(conv1x1(in_dim, hidden_dim))
        layers.extend([
            #dw
            dwise_conv(hidden_dim, stride=stride),
            #pw
            conv1x1(hidden_dim, out_dim)
        ])
...
class MobileNetV2(nn.Module):
    def __init__(self, in_dim=12, n_classes=9):
        ...
        #initial layer 
        self.stem_conv = conv3x3(in_dim, 32, stride=2)
        ...
        for t, c, n, s in self.configs:
            for i in range(n):
                ...
                layers.append(Bottleneck(in_dim=input_channel, out_dim=c, expand_ratio=t, stride=stride))
                input_channel = c
